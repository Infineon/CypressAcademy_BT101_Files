 ...
 * Function Prototypes
 ******************************************************************/
static void                   ex04_ble_con_app_init               ( void );
static wiced_bt_dev_status_t  ex04_ble_con_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   ex04_ble_con_set_advertisement_data ( void );
static void                   ex04_ble_con_advertisement_stopped  ( void );
static void                   ex04_ble_con_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t ex04_ble_con_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex04_ble_con_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex04_ble_con_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t ex04_ble_con_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t ex04_ble_con_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
static uint32_t               hci_control_process_rx_cmd          ( uint8_t* p_data, uint32_t len );
#ifdef HCI_TRACE_OVER_TRANSPORT
static void                   ex04_ble_con_trace_callback         ( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif
 ...
 ******************************************************************/
uint8_t ex04_ble_con_generic_access_device_name[] = {'k','e','y','_','c','o','n'};
uint8_t ex04_ble_con_generic_access_appearance[]  = {0x00,0x00};
uint8_t ex04_ble_con_wiced101_led[]               = {0x00};
uint8_t ex04_ble_con_wiced101_button[]            = {0x00};
...
gatt_db_lookup_table ex04_ble_con_gatt_db_ext_attr_tbl[] =
{
    /* { attribute handle,                  maxlen, curlen, attribute data } */
    {HDLC_GENERIC_ACCESS_DEVICE_NAME_VALUE, 7,      7,     ex04_ble_con_generic_access_device_name},
    {HDLC_GENERIC_ACCESS_APPEARANCE_VALUE,  2,      2,      ex04_ble_con_generic_access_appearance},
    {HDLC_WICED101_LED_VALUE,               1,      1,      ex04_ble_con_wiced101_led},
    {HDLC_WICED101_BUTTON_VALUE,            1,      1,      ex04_ble_con_wiced101_button},
};

// Number of Lookup Table Entries
const uint16_t ex04_ble_con_gatt_db_ext_attr_tbl_size = ( sizeof ( ex04_ble_con_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
void application_start(void)
{
    ...
    wiced_bt_stack_init(ex04_ble_con_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);
}

/*
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void ex04_ble_con_app_init(void)
{
    ...
    ex04_ble_con_set_advertisement_data();

    /* Register with stack to receive GATT callback */
    wiced_bt_gatt_register( ex04_ble_con_event_handler );
...
/* Set Advertisement Data */
void ex04_ble_con_set_advertisement_data( void )
{
...
void ex04_ble_con_advertisement_stopped( void )
{
...
void ex04_ble_con_reset_device( void )
{
...
wiced_bt_dev_status_t ex04_ble_con_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(ex04_ble_con_trace_callback);
#endif
...
#if 0
        if (ex04_ble_con_read_link_keys( &p_event_data->paired_device_link_keys_request ))
        {
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            ex04_ble_con_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t ex04_ble_con_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < ex04_ble_con_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex04_ble_con_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (ex04_ble_con_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = ex04_ble_con_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, ex04_ble_con_gatt_db_ext_attr_tbl[i].p_data, ex04_ble_con_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t ex04_ble_con_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < ex04_ble_con_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex04_ble_con_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (ex04_ble_con_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                ex04_ble_con_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(ex04_ble_con_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
                ...
                case HDLC_WICED101_LED_VALUE:
                    ...
                    wiced_hal_gpio_set_pin_output(WICED_GPIO_PIN_LED_2, !ex04_ble_con_wiced101_led[0]);
                    WICED_BT_TRACE("Output = %d\n", ex04_ble_con_wiced101_led[0]);
                    break;
                }
            }
            else
...
wiced_bt_gatt_status_t ex04_ble_con_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = ex04_ble_con_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t ex04_ble_con_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = ex04_ble_con_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t ex04_ble_con_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
...
wiced_bt_gatt_status_t ex04_ble_con_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = ex04_ble_con_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = ex04_ble_con_write_handler( &p_data->write_req, conn_id );
        break;
    }
...
/* GATT Event Handler */
wiced_bt_gatt_status_t ex04_ble_con_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = ex04_ble_con_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = ex04_ble_con_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
void ex04_ble_con_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
...
void button_cback( void *data, uint8_t port_pin )
{
    ...
    ex04_ble_con_wiced101_button[0] = !wiced_hal_gpio_get_pin_input_status( WICED_GPIO_PIN_BUTTON_1 );
}
