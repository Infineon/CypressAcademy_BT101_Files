...
void app_set_advertisement_data( void )
{
    ...
    uint8_t eddy_uuid[] = { EDDY_UUID_LSB, EDDY_UUID_MSB};
    uint8_t eddy_data[] =
    {
		EDDY_UUID_LSB,
		EDDY_UUID_MSB,
    ...
    adv_elem[num_elem].len = sizeof( eddy_uuid );
    adv_elem[num_elem].p_data = eddy_uuid;
    num_elem++;

    /* Advertisement Element for Service Data */
    adv_elem[num_elem].advert_type = BTM_BLE_ADVERT_TYPE_SERVICE_DATA;
    adv_elem[num_elem].len = sizeof( eddy_data );
    adv_elem[num_elem].p_data = eddy_data;
    num_elem++;
