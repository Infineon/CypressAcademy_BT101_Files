...
uint8_t ex01_low_power_capsense_buttons[]           = {0x04,0x00,0x00};
uint8_t ex01_low_power_capsense_buttons_cccd[]      = {0x00,0x00};
...
gatt_db_lookup_table ex01_low_power_gatt_db_ext_attr_tbl[] =
{
    ...
    {HDLC_CAPSENSE_BUTTONS_VALUE,                3,      3,      ex01_low_power_capsense_buttons},
    {HDLD_CAPSENSE_BUTTONS_CLIENT_CONFIGURATION, 2,      2,      ex01_low_power_capsense_buttons_cccd},
};
...
wiced_bt_dev_status_t ex01_low_power_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
        ...
        if(bond_mode == WICED_FALSE)
        {
            ...
            ex01_low_power_capsense_buttons_cccd[0] = hostinfo.cccd;
...
wiced_bt_gatt_status_t ex01_low_power_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    if ( NULL != p_conn_status )
    {
        ...
        else
        {
            ...
            ex01_low_power_capsense_buttons_cccd[0] = 0;
...
void i2c_read( uint32_t arg )
{
    ...
    for(;;)
    {
        ...
        if(prevVal != buttonVal) /* Only print if value has changed since last time */
        {
            ...
            ex01_low_power_capsense_buttons[2] = buttonVal;
            /* If the connection is up and if the client wants notifications, send it */
            if ( connection_id != 0)
            {
                 if(ex01_low_power_capsense_buttons_cccd[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
                {
                    wiced_bt_gatt_send_notification(connection_id, HDLC_CAPSENSE_BUTTONS_VALUE, sizeof(ex01_low_power_capsense_buttons), ex01_low_power_capsense_buttons );
