...
wiced_sleep_config_t sleep_cfg;
    ...
    switch( event )
    {
		case BTM_ENABLED_EVT:								// Bluetooth Controller and Host Stack Enabled
			if( WICED_BT_SUCCESS == p_event_data->enabled.status )
			{
				...
				sleep_cfg.device_wake_mode			= WICED_SLEEP_WAKE_ACTIVE_LOW;		// Unused if sleep_mode = WICED_SLEEP_MODE_NO_TRANSPORT
				sleep_cfg.device_wake_source		= WICED_SLEEP_WAKE_SOURCE_GPIO;
				sleep_cfg.device_wake_gpio_num		= WICED_GPIO_PIN_BUTTON_1;
				sleep_cfg.host_wake_mode			= WICED_SLEEP_WAKE_ACTIVE_LOW;		// Unused if sleep_mode = WICED_SLEEP_MODE_NO_TRANSPORT
    ...
    switch( event )
    {
        case GATT_CONNECTION_STATUS_EVT:					// Remote device initiates connect/disconnect
            if( p_conn->connected )
			{
	            ...
	            /* Request update of connection params to:
	             *   min interval = 200 * 1.25 = 250 ms
	             *   max interval = 200 * 1.25 = 250 ms
	             *   latency = 3
	             *   timeout = 512 * 10 = 5120 ms
	             */
	            wiced_bt_l2cap_update_ble_conn_params( p_conn->bd_addr, 200, 200, 3, 512 );
			}
			else
			{
...
//		wiced_hal_pwm_change_values( PWM0, PWM_BONDING_TOGGLE, PWM_BONDING_INIT );
		...
		wiced_hal_write_nvram( VSID_HOSTINFO, sizeof(hostinfo), (uint8_t*)&hostinfo, &result );
		wiced_hal_write_nvram ( VSID_REMOTE_KEY, sizeof( wiced_bt_device_link_keys_t ), (uint8_t*)&keys, &result );
    }
}


/*******************************************************************************
* Function Name: low_power_sleep_callback( wiced_sleep_poll_type_t type )
*
* Low power sleep callback function that the stack calls when it wants to
* determine if sleep can be done.
********************************************************************************/
uint32_t low_power_sleep_callback( wiced_sleep_poll_type_t type )
{
	uint32_t ret = WICED_ERROR;
	...
	switch( type )
	{
		case WICED_SLEEP_POLL_SLEEP_PERMISSION:
			...
			WICED_BT_TRACE( "." ); // Print . when sleep permission is requested
			break;
