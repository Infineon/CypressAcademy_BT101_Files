 ...
 * Function Prototypes
 ******************************************************************/
static void                   ex07_low_power_sds_app_init               ( void );
static wiced_bt_dev_status_t  ex07_low_power_sds_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   ex07_low_power_sds_set_advertisement_data ( void );
static void                   ex07_low_power_sds_advertisement_stopped  ( void );
static void                   ex07_low_power_sds_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t ex07_low_power_sds_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex07_low_power_sds_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex07_low_power_sds_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t ex07_low_power_sds_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t ex07_low_power_sds_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
...
gatt_db_lookup_table ex07_low_power_sds_gatt_db_ext_attr_tbl[] =
{
...
const uint16_t ex07_low_power_sds_gatt_db_ext_attr_tbl_size = ( sizeof ( ex07_low_power_sds_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
void application_start(void)
{
    ...
    wiced_bt_stack_init(ex07_low_power_sds_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);
 ...
 */
void ex07_low_power_sds_app_init(void)
{
    ...
    ex07_low_power_sds_set_advertisement_data();
    ...
    wiced_bt_gatt_register( ex07_low_power_sds_event_handler );
...
/* Set Advertisement Data */
void ex07_low_power_sds_set_advertisement_data( void )
{
...
void ex07_low_power_sds_advertisement_stopped( void )
{
...
void ex07_low_power_sds_reset_device( void )
{
...
wiced_bt_dev_status_t ex07_low_power_sds_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(ex07_low_power_sds_trace_callback);
#endif
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            ex07_low_power_sds_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t ex07_low_power_sds_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < ex07_low_power_sds_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex07_low_power_sds_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (ex07_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = ex07_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, ex07_low_power_sds_gatt_db_ext_attr_tbl[i].p_data, ex07_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t ex07_low_power_sds_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < ex07_low_power_sds_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex07_low_power_sds_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (ex07_low_power_sds_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                ex07_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(ex07_low_power_sds_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
...
wiced_bt_gatt_status_t ex07_low_power_sds_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = ex07_low_power_sds_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t ex07_low_power_sds_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = ex07_low_power_sds_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t ex07_low_power_sds_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
...
wiced_bt_gatt_status_t ex07_low_power_sds_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = ex07_low_power_sds_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = ex07_low_power_sds_write_handler( &p_data->write_req, conn_id );
        break;
    }
    return status;
}
...
/* GATT Event Handler */
wiced_bt_gatt_status_t ex07_low_power_sds_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = ex07_low_power_sds_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = ex07_low_power_sds_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
void ex07_low_power_sds_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
