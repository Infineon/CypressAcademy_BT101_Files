...
#endif
	wiced_bt_cfg_settings.device_name = (uint8_t *)"HSL Key";
...
{
    WICED_BT_TRACE("GJL: Element:%d Event:%d\n", element_idx, event);
...
void mesh_app_process_set_hsl(uint8_t element_idx, wiced_bt_mesh_light_hsl_status_data_t *p_status)
{
    ...
    led_control_set_brightness_level(p_status->present.hue, p_status->present.saturation, p_status->present.lightness);
}
