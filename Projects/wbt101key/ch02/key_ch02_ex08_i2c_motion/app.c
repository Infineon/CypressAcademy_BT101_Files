...
void app_task( uint32_t arg )
{
	...
	wiced_hal_i2c_write( accelConfig, sizeof(accelConfig), ACCEL_ADDRESS );
	...
	while( 1 )
	{
		...
		wiced_hal_i2c_combined_read( (uint8_t*)&accelData, sizeof(accelData), &accelDataReg, sizeof(accelDataReg), ACCEL_ADDRESS );
