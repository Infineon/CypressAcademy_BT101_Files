...
wiced_result_t app_bt_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:								// Bluetooth Controller and Host Stack Enabled
        if( WICED_BT_SUCCESS == p_event_data->enabled.status )
        {
        	...
        	/* Report the BDA to ensure it is random */
        ...
        {
        	WICED_BT_TRACE( "Bluetooth stack error\r\n" );
        }
        break;
...
wiced_bt_gatt_status_t app_gatt_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data )
{
    ...
    switch( event )
    {
        ...
        case GATT_ATTRIBUTE_REQUEST_EVT:					// Remote device initiates a GATT read/write
        	// WICED_BT_TRACE( "GATT_ATTRIBUTE_REQUEST_EVT %d %d\r\n", attr->request_type, attr->data.handle );
...
void app_set_advertisement_data( void )
{
    ...
    adv_elem[num_elem].len = sizeof( uuid );
    adv_elem[num_elem].p_data = uuid;
    num_elem++;
...
void button_cback( void *data, uint8_t port_pin )
{
	...
	WICED_BT_TRACE( "ccd = %x %x\r\n", app_modus_counter_client_char_config[0], app_modus_counter_client_char_config[1] );
