...
#include "ex05_low_power_sds_db.h"
 ...
 * Function Prototypes
 ******************************************************************/
static void                   ex05_low_power_sds_app_init               ( void );
static wiced_bt_dev_status_t  ex05_low_power_sds_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   ex05_low_power_sds_set_advertisement_data ( void );
static void                   ex05_low_power_sds_advertisement_stopped  ( void );
static void                   ex05_low_power_sds_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t ex05_low_power_sds_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex05_low_power_sds_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex05_low_power_sds_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t ex05_low_power_sds_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t ex05_low_power_sds_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
static uint32_t               hci_control_process_rx_cmd          ( uint8_t* p_data, uint32_t len );
#ifdef HCI_TRACE_OVER_TRANSPORT
static void                   ex05_low_power_sds_trace_callback         ( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif
 ...
 ******************************************************************/
uint8_t ex05_low_power_sds_generic_access_device_name[] = {'k','e','y','_','l','p'};
uint8_t ex05_low_power_sds_generic_access_appearance[]  = {0x00,0x00};
uint8_t ex05_low_power_sds_capsense_buttons[]           = {0x04,0x00,0x00};
uint8_t ex05_low_power_sds_capsense_buttons_cccd[]      = {0x00,0x00};
...
gatt_db_lookup_table ex05_low_power_sds_gatt_db_ext_attr_tbl[] =
{
    /* { attribute handle,                       maxlen, curlen, attribute data } */
    {HDLC_GENERIC_ACCESS_DEVICE_NAME_VALUE,      6,      6,      ex05_low_power_sds_generic_access_device_name},
    {HDLC_GENERIC_ACCESS_APPEARANCE_VALUE,       2,      2,      ex05_low_power_sds_generic_access_appearance},
    {HDLC_CAPSENSE_BUTTONS_VALUE,                3,      3,      ex05_low_power_sds_capsense_buttons},
    {HDLD_CAPSENSE_BUTTONS_CLIENT_CONFIGURATION, 2,      2,      ex05_low_power_sds_capsense_buttons_cccd},
};

// Number of Lookup Table Entries
const uint16_t ex05_low_power_sds_gatt_db_ext_attr_tbl_size = ( sizeof ( ex05_low_power_sds_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
void application_start(void)
{
    ...
    wiced_bt_stack_init(ex05_low_power_sds_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);
    ...
    /* Check if Cold Boot or Warm (Fast) Boot (i.e. initial powerup/reset or wake from SDS) */
    low_power_boot_mode = wiced_sleep_get_boot_mode();
}

/*
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void ex05_low_power_sds_app_init(void)
{
    ...
    else // Warm (Fast) Boot - just came out of SDS
    {
        ...
        if(0 != connection_id) /* Already connected so we need to read the connection info from NVRAM */
        {
            ...
            ex05_low_power_sds_capsense_buttons_cccd[0] = hostinfo.cccd;

            /* Set CapSense button value in the GATT array from the prevVal which is stored in AON RAM */
            ex05_low_power_sds_capsense_buttons[2] = prevVal;
        }
    }
    ...
    ex05_low_power_sds_set_advertisement_data();
    ...
    if(result == WICED_BT_SUCCESS)
    {
        ...
        if (WICED_SLEEP_COLD_BOOT == low_power_boot_mode) // Only print on first time boot
        {
    ...
    wiced_bt_gatt_register( ex05_low_power_sds_event_handler );
    ...
    if(0 == connection_id)
    {
        ...
        if(BTM_BLE_ADVERT_OFF != advert_mode) /* Restart only if they have not timed out */
        {
            if (WICED_SLEEP_COLD_BOOT == low_power_boot_mode) // Only print on first time boot
            {
                WICED_BT_TRACE( "Start Advertisements \n\r");
            }
                wiced_bt_start_advertisements(advert_mode, 0, NULL);
        }
        sleep_type = WICED_SLEEP_ALLOWED_WITH_SHUTDOWN;
    }
    else
    {
        /* Warm (Fast) boot since we were already connected */
        sleep_type = WICED_SLEEP_ALLOWED_WITH_SHUTDOWN;
    }
}

/* Set Advertisement Data */
void ex05_low_power_sds_set_advertisement_data( void )
{
...
void ex05_low_power_sds_advertisement_stopped( void )
{
...
void ex05_low_power_sds_reset_device( void )
{
...
wiced_bt_dev_status_t ex05_low_power_sds_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(ex05_low_power_sds_trace_callback);
#endif
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
        ...
        {
            sleep_type = WICED_SLEEP_ALLOWED_WITH_SHUTDOWN;
            WICED_BT_TRACE("Enable Sleep Type: %d\n\r", sleep_type);
        }
        break;
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        if(advert_mode != *p_adv_mode)
        {
            /* Only print if changed */
            advert_mode = *p_adv_mode; /* Keep track of mode so we can restore after SDS */
            WICED_BT_TRACE("Advertisement State Change: %d\n", *p_adv_mode);
        }
        if ( BTM_BLE_ADVERT_OFF == *p_adv_mode )
        {
            ex05_low_power_sds_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t ex05_low_power_sds_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < ex05_low_power_sds_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex05_low_power_sds_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (ex05_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = ex05_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, ex05_low_power_sds_gatt_db_ext_attr_tbl[i].p_data, ex05_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t ex05_low_power_sds_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < ex05_low_power_sds_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex05_low_power_sds_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (ex05_low_power_sds_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                ex05_low_power_sds_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(ex05_low_power_sds_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
...
wiced_bt_gatt_status_t ex05_low_power_sds_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = ex05_low_power_sds_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t ex05_low_power_sds_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = ex05_low_power_sds_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t ex05_low_power_sds_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    if ( NULL != p_conn_status )
    {
        ...
        {
            /* Switch to PDS instead of SDS when we first connect so that encryption can be done */
            sleep_type = WICED_SLEEP_ALLOWED_WITHOUT_SHUTDOWN;
        ...
        else
        {
            ...
            ex05_low_power_sds_capsense_buttons_cccd[0] = 0;
...
wiced_bt_gatt_status_t ex05_low_power_sds_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = ex05_low_power_sds_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = ex05_low_power_sds_write_handler( &p_data->write_req, conn_id );
        break;
    }
    return status;
}
...
/* GATT Event Handler */
wiced_bt_gatt_status_t ex05_low_power_sds_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = ex05_low_power_sds_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = ex05_low_power_sds_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
void ex05_low_power_sds_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
...
void i2c_read( uint32_t arg )
{
    ...
    for(;;)
    {
        ...
        if(prevVal != buttonVal) /* Only print if value has changed since last time */
        {
            ...
            ex05_low_power_sds_capsense_buttons[2] = buttonVal;
            /* If the connection is up and if the client wants notifications, send it */
            if ( connection_id != 0)
            {
                 if(ex05_low_power_sds_capsense_buttons_cccd[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
                {
                    wiced_bt_gatt_send_notification(connection_id, HDLC_CAPSENSE_BUTTONS_VALUE, sizeof(ex05_low_power_sds_capsense_buttons), ex05_low_power_sds_capsense_buttons );
...
uint32_t low_power_sleep_callback(wiced_sleep_poll_type_t type )
{
    uint32_t ret = WICED_SLEEP_NOT_ALLOWED; /* Assume sleep not allowed */

    switch(type)
    {
        case WICED_SLEEP_POLL_SLEEP_PERMISSION:
            ret = sleep_type;
            if(WICED_SLEEP_ALLOWED_WITH_SHUTDOWN == sleep_type)
            {
                WICED_BT_TRACE("."); /* Dot means sleep allowed with shutdown */
            }
            else if (WICED_SLEEP_ALLOWED_WITHOUT_SHUTDOWN == sleep_type)
            {
                WICED_BT_TRACE("$"); /* Dollar means sleep allowed without shutdown */
            }
            else
            {
                WICED_BT_TRACE("x"); /* x means sleep not allowed */
            }
            break;
