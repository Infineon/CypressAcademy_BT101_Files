...
/* This project supports saving info for multiple bonded devices. */
/* MB1 is used to toggle bonding mode. The device must be in bonding mode to pair a new device. */
/* At initial power-up, the device will start in bonding mode */
/* If there is no space for more devices, entering bonding mode will erase the oldest set of bonding info */
/* When MB2 is pressed, the device will dump out a list of bonded devices */
...
/* WICED_NVRAM_LOCAL_KEYS holds the privacy keys */
...
#define WICED_NVRAM_LOCAL_KEYS         ( WICED_NVRAM_VSID_START )
#define WICED_NVRAM_BOND_INFO          ( WICED_NVRAM_LOCAL_KEYS + 1 )
#define WICED_NVRAM_HOST_INFO0         ( WICED_NVRAM_BOND_INFO + 1 )
#define WICED_NVRAM_PAIRED_KEYS0       ( WICED_NVRAM_HOST_INFO0 + BOND_MAX )
...
wiced_bool_t bondMode = WICED_TRUE; // If true we will go into bonding mode to add a new device (if there is space). We start out in bond mode since we have no bonding info to start
 ...
 * Function Prototypes
 ******************************************************************/
static void                   ex07_ble_multi_app_init               ( void );
static wiced_bt_dev_status_t  ex07_ble_multi_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   ex07_ble_multi_set_advertisement_data ( void );
static void                   ex07_ble_multi_advertisement_stopped  ( void );
static void                   ex07_ble_multi_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t ex07_ble_multi_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex07_ble_multi_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex07_ble_multi_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t ex07_ble_multi_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t ex07_ble_multi_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
static uint32_t               hci_control_process_rx_cmd          ( uint8_t* p_data, uint32_t len );
#ifdef HCI_TRACE_OVER_TRANSPORT
static void                   ex07_ble_multi_trace_callback         ( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif

void ledBlinkCallback(uint32_t arg);
void i2c_read( uint32_t arg );
void button_cback1( void *data, uint8_t port_pin );  // Button ISR to enter bonding mode
void button_cback2( void *data, uint8_t port_pin );  // Button ISR to enter bonding mode
 ...
 ******************************************************************/
uint8_t ex07_ble_multi_generic_access_device_name[] = {'k','e','y','_','m','u','l','t'};
uint8_t ex07_ble_multi_generic_access_appearance[]  = {0x00,0x00};
uint8_t ex07_ble_multi_wiced101_led[]               = {0x00};
uint8_t ex07_ble_multi_wiced101_buttons[]           = {0x04,0x00,0x00};
uint8_t ex07_ble_multi_wiced101_buttons_cccd[]      = {0x00,0x00};
...
gatt_db_lookup_table ex07_ble_multi_gatt_db_ext_attr_tbl[] =
{
    /* { attribute handle,                       maxlen, curlen, attribute data } */
    {HDLC_GENERIC_ACCESS_DEVICE_NAME_VALUE,      8,      8,      ex07_ble_multi_generic_access_device_name},
    {HDLC_GENERIC_ACCESS_APPEARANCE_VALUE,       2,      2,      ex07_ble_multi_generic_access_appearance},
    {HDLC_WICED101_LED_VALUE,                    1,      1,      ex07_ble_multi_wiced101_led},
    {HDLC_WICED101_BUTTONS_VALUE,                3,      3,      ex07_ble_multi_wiced101_buttons},
    {HDLD_WICED101_BUTTONS_CLIENT_CONFIGURATION, 2,      2,      ex07_ble_multi_wiced101_buttons_cccd},
};

// Number of Lookup Table Entries
const uint16_t ex07_ble_multi_gatt_db_ext_attr_tbl_size = ( sizeof ( ex07_ble_multi_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
void application_start(void)
{
    ...
    wiced_bt_stack_init(ex07_ble_multi_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);
}

/*
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void ex07_ble_multi_app_init(void)
{
    uint8_t count;
    ...
    /* Configure the Button GPIOs as inputs with a resistive pull up and interrupt on rising edge */
    wiced_hal_gpio_register_pin_for_interrupt( WICED_GPIO_PIN_BUTTON_1, button_cback1, NULL );
    ...
    wiced_hal_gpio_register_pin_for_interrupt( WICED_GPIO_PIN_BUTTON_2, button_cback2, NULL );
    ...
    ex07_ble_multi_set_advertisement_data();
    ...
    if(result == WICED_BT_SUCCESS)
    {
        if(bondInfo[NUM_BONDED] > 0)
        {
            ...
            for(count = 0; count < bondInfo[NUM_BONDED]; count++)
            {
                ...
                p = (uint8_t*)&link_keys;
                wiced_hal_read_nvram( WICED_NVRAM_PAIRED_KEYS0 + count, sizeof(wiced_bt_device_link_keys_t), p, &result);
            ...
            bondMode = WICED_FALSE;    /* We have bonding information already, so don't go into bonding mode */
        }
    }
    WICED_BT_TRACE("Number of bonded devices: %d, Next free slot: %d\n",bondInfo[NUM_BONDED], bondInfo[NEXT_FREE]);

    /* Register with stack to receive GATT callback */
    wiced_bt_gatt_register( ex07_ble_multi_event_handler );
    ...
    wiced_start_timer(&ledBlinkTimer, bondMode ? BONDING_BLINK_RATE : BONDED_BLINK_RATE);
...
/* Set Advertisement Data */
void ex07_ble_multi_set_advertisement_data( void )
{
...
void ex07_ble_multi_advertisement_stopped( void )
{
...
void ex07_ble_multi_reset_device( void )
{
...
wiced_bt_dev_status_t ex07_ble_multi_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(ex07_ble_multi_trace_callback);
#endif
    ...
    case BTM_PAIRING_COMPLETE_EVT:
        ...
        if ( p_ble_info->reason == WICED_BT_SUCCESS ) /* Bonding successful */
        {
            ...
            bondMode = WICED_FALSE; // Exit bonding mode
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
        ...
        for(count = 0; count < bondInfo[NUM_BONDED]; count++)
        {
            ...
            if( memcmp(hostinfo.bdaddr,hostinfoTemp.bdaddr,sizeof(hostinfo.bdaddr) ) == 0) /* Matching address */
            {
                ...
                ex07_ble_multi_wiced101_buttons_cccd[0] = hostinfo.cccd; /* Set CCCD value from the value that was previously saved in the NVRAM */
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            ex07_ble_multi_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t ex07_ble_multi_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < ex07_ble_multi_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex07_ble_multi_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (ex07_ble_multi_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = ex07_ble_multi_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, ex07_ble_multi_gatt_db_ext_attr_tbl[i].p_data, ex07_ble_multi_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t ex07_ble_multi_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < ex07_ble_multi_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex07_ble_multi_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (ex07_ble_multi_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                ex07_ble_multi_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(ex07_ble_multi_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
                ...
                case HDLC_WICED101_LED_VALUE:
                    ...
                    WICED_BT_TRACE("Output = %d\n", ex07_ble_multi_wiced101_led[0]);
                    wiced_hal_gpio_set_pin_output(WICED_GPIO_PIN_LED_2, ex07_ble_multi_wiced101_led[0]);
                    break;
                case HDLD_WICED101_BUTTONS_CLIENT_CONFIGURATION:
                    ...
                    wiced_hal_write_nvram( WICED_NVRAM_HOST_INFO0 + bondIndex, sizeof(hostinfo), (uint8_t*)&hostinfo, &temp_result );
                    WICED_BT_TRACE("\t\tWrite CCCD value to NVRAM for bonded device %d\n\r",bondIndex);
                    break;
                }
            }
            else
...
wiced_bt_gatt_status_t ex07_ble_multi_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = ex07_ble_multi_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t ex07_ble_multi_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = ex07_ble_multi_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t ex07_ble_multi_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    if ( NULL != p_conn_status )
    {
        ...
        else
        {
            ...
            ex07_ble_multi_wiced101_buttons_cccd[0] = 0;

            /* set timer to correct speed depending on bondMode and restart the advertisements */
            wiced_init_timer(&ledBlinkTimer, ledBlinkCallback, 0, WICED_MILLI_SECONDS_PERIODIC_TIMER);
            wiced_start_timer(&ledBlinkTimer, bondMode ? BONDING_BLINK_RATE : BONDED_BLINK_RATE);
            wiced_bt_start_advertisements(BTM_BLE_ADVERT_UNDIRECTED_HIGH, 0, NULL);
        }
...
wiced_bt_gatt_status_t ex07_ble_multi_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = ex07_ble_multi_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = ex07_ble_multi_write_handler( &p_data->write_req, conn_id );
        break;
    }
...
/* GATT Event Handler */
wiced_bt_gatt_status_t ex07_ble_multi_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = ex07_ble_multi_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = ex07_ble_multi_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
void ex07_ble_multi_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
...
void i2c_read( uint32_t arg )
{
    ...
    for(;;)
    {
        ...
        buttonVal = i2cReg & WICED101_MASK;

        if(prevVal != buttonVal) /* Only print if value has changed since last time */
        {
            WICED_BT_TRACE( "Button State: %02X\n\r", buttonVal);
            ex07_ble_multi_wiced101_buttons[2] = buttonVal;
            /* If the connection is up and if the client wants notifications, send it */
            if ( connection_id != 0)
            {
                 if(ex07_ble_multi_wiced101_buttons_cccd[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
                {
                    wiced_bt_gatt_send_notification(connection_id, HDLC_WICED101_BUTTONS_VALUE, sizeof(ex07_ble_multi_wiced101_buttons), ex07_ble_multi_wiced101_buttons );
...
/* Interrupt callback function for BUTTON_1  - toggle bonding mode */
/* If bonding space is full, remove the oldest set of info before entering bonding mode */
void button_cback1( void *data, uint8_t port_pin )
{
    wiced_result_t                  result;
    wiced_bt_device_link_keys_t     link_keys;
    ...
    if(bondMode == WICED_FALSE) /* Enter bond mode */
    {
...
void button_cback2( void *data, uint8_t port_pin )
{
    uint8_t count;
