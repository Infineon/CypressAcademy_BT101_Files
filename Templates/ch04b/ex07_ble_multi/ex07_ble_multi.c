...
uint8_t ex07_ble_multi_capsense_buttons[]           = {0x04,0x00,0x00};
uint8_t ex07_ble_multi_capsense_buttons_cccd[]      = {0x00,0x00};
...
gatt_db_lookup_table ex07_ble_multi_gatt_db_ext_attr_tbl[] =
{
    ...
    {HDLC_GENERIC_ACCESS_DEVICE_NAME_VALUE,      12,     12,     ex07_ble_multi_generic_access_device_name},
    {HDLC_GENERIC_ACCESS_APPEARANCE_VALUE,       2,      2,      ex07_ble_multi_generic_access_appearance},
    {HDLC_CAPSENSE_BUTTONS_VALUE,                3,      3,      ex07_ble_multi_capsense_buttons},
    {HDLD_CAPSENSE_BUTTONS_CLIENT_CONFIGURATION, 2,      2,      ex07_ble_multi_capsense_buttons_cccd},
};
...
void ex07_ble_multi_app_init(void)
{
    ...
    /* Start a thread to read button values */
     ...
     /* Start a thread to blink LED if we are in bonding mode */
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
        ...
        for(count = 0; count < bondInfo[NUM_BONDED]; count++)
        {
            ...
            if( memcmp(hostinfo.bdaddr,hostinfoTemp.bdaddr,sizeof(hostinfo.bdaddr) ) == 0) /* Matching address */
            {
                ...
                ex07_ble_multi_capsense_buttons_cccd[0] = hostinfo.cccd; /* Set CCCD value from the value that was previously saved in the NVRAM */
...
wiced_bt_gatt_status_t ex07_ble_multi_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    if ( NULL != p_conn_status )
    {
        ...
        else
        {
            ...
            ex07_ble_multi_capsense_buttons_cccd[0] = 0;
...
void i2c_read( uint32_t arg )
{
    ...
    for(;;)
    {
        ...
        if(prevVal != buttonVal) /* Only print if value has changed since last time */
        {
            ...
            ex07_ble_multi_capsense_buttons[2] = buttonVal;
            /* If the connection is up and if the client wants notifications, send it */
            if ( connection_id != 0)
            {
                 if(ex07_ble_multi_capsense_buttons_cccd[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
                {
                    wiced_bt_gatt_send_notification(connection_id, HDLC_CAPSENSE_BUTTONS_VALUE, sizeof(ex07_ble_multi_capsense_buttons), ex07_ble_multi_capsense_buttons );
